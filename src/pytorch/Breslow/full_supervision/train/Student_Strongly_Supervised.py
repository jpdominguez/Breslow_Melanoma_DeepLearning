...
models_path = 'E:\\Breslow\\src\\pytorch\\experiments\\Breslow\\full_supervision\\train\\models\\'
...
CSV_PATH_VDR = 'E:\\Breslow\\melanomas virgen del rocio\\Total de imagenes con Breslow-VR\\images_paths_whole.csv'
CSV_PATH_ARG = 'E:\\Breslow\\Argenciano\\images_paths_whole.csv'
CSV_PATH_ISIC = 'E:\\Breslow\\ISIC con Breslow 27-01-23\\images3\\isic_test.csv'
CSV_PATH_POLESIE = 'E:\\Breslow\\Imagenes Polesie et al 2021\\images_paths_whole.csv'
paths_folder = 'E:\\PROSTSET\\partitions\\'
...
csv_df_vdr = csv_df_vdr[csv_df_vdr.BRESLOW != 'Tis']
csv_df_vdr = csv_df_vdr[csv_df_vdr.BRESLOW != 'Tis '].reset_index()

csv_df_vdr['BRESLOW'] = csv_df_vdr['BRESLOW'].str.replace(',', '.').astype(float)

breslow_processed_vdr = []
for i in range(len(csv_df_vdr)):
    if float(csv_df_vdr.iloc[i]['BRESLOW']) < 0.8:
        breslow_processed_vdr.append(0)
    else:
        breslow_processed_vdr.append(1)
csv_df_vdr['BRESLOW_processed'] = breslow_processed_vdr
...
print('Breslow < 0.8:', len(csv_df_vdr_shuffled[csv_df_vdr_shuffled.BRESLOW_processed == 0]))
print('Breslow >= 0.8:', len(csv_df_vdr_shuffled[csv_df_vdr_shuffled.BRESLOW_processed == 1]))
print('------------------------')
...
csv_df_arg = csv_df_arg[csv_df_arg.BRESLOW_processed != 'X']
csv_df_arg = csv_df_arg[csv_df_arg.BRESLOW_processed != '-'].reset_index()
csv_df_arg['BRESLOW_processed'] = csv_df_arg['BRESLOW_processed'].str.replace(',', '.').astype(int)
...
print('Breslow < 0.8:', len(csv_df_arg_shuffled[csv_df_arg_shuffled.BRESLOW_processed < 0.8]))
print('Breslow >= 0.8:', len(csv_df_arg_shuffled[csv_df_arg_shuffled.BRESLOW_processed >= 0.8]))
print('------------------------')
...
csv_df_polesie = csv_df_polesie[csv_df_polesie.BRESLOW_processed != 'X']
csv_df_polesie = csv_df_polesie[csv_df_polesie.BRESLOW_processed != '-'].reset_index()
csv_df_polesie['BRESLOW_processed'] = csv_df_polesie['BRESLOW_processed'].str.replace(',', '.').astype(int)
...
print('Breslow < 0.8:', len(csv_df_polesie_shuffled[csv_df_polesie_shuffled.BRESLOW_processed < 0.8]))
print('Breslow >= 0.8:', len(csv_df_polesie_shuffled[csv_df_polesie_shuffled.BRESLOW_processed >= 0.8]))
print('------------------------')
...
for i in range(len(csv_df_isic)):
    if float(csv_df_isic.iloc[i]['mel_thick_mm']) < 0.8:
        breslow_processed.append(0)
    else:
        breslow_processed.append(1)
csv_df_isic['BRESLOW_processed'] = breslow_processed
...
print('ISIC')
print('Images found:', len(csv_df_isic_shuffled), '--', np.all([os.path.isfile(i) for i in csv_df_isic_shuffled_iloc['IMAGE_PATH']]))
print('Breslow < 0.8:', len(csv_df_isic_shuffled[csv_df_isic_shuffled.BRESLOW_processed < 0.8]))
print('Breslow >= 0.8:', len(csv_df_isic_shuffled[csv_df_isic_shuffled.BRESLOW_processed >= 0.8]))
print('------------------------')



csv_df_vdr_shuffled['BRESLOW_processed'] = [str(x) for x in csv_df_vdr_shuffled['BRESLOW_processed']]
csv_df_arg_shuffled['BRESLOW_processed'] = [str(x) for x in csv_df_arg_shuffled['BRESLOW_processed']]
csv_df_polesie_shuffled['BRESLOW_processed'] = [str(x) for x in csv_df_polesie_shuffled['BRESLOW_processed']]
csv_df_isic_shuffled['BRESLOW_processed'] = [str(x) for x in csv_df_isic_shuffled['BRESLOW_processed']]
...
for i in range(5):
	...
	train_df.to_csv(OUTPUT_PATH_FOLD+'train_df.csv', index = False, header=True, columns=['IMAGE_PATH','BRESLOW_processed'])
	test_df.to_csv(OUTPUT_PATH_FOLD+'validation_df.csv', index = False, header=True, columns=['IMAGE_PATH','BRESLOW_processed'])
	...
	training_set = Dataset_train(train_dataset[:,2], train_dataset[:,16])
	training_generator = data.DataLoader(training_set, **params_train)

	validation_set = Dataset_test(valid_dataset[:,2], valid_dataset[:,16])

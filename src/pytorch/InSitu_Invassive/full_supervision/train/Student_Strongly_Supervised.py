...
Tis_inv_processed_vdr = []
for i in range(len(csv_df_vdr)):
    if csv_df_vdr.iloc[i]['BRESLOW'] == 'Tis':
        Tis_inv_processed_vdr.append(1)
    else:
        Tis_inv_processed_vdr.append(0)
csv_df_vdr['Tis_inv'] = Tis_inv_processed_vdr
...
print('In situ:', len(csv_df_vdr_shuffled[csv_df_vdr_shuffled.Tis_inv == 1]))
print('Invassive:', len(csv_df_vdr_shuffled[csv_df_vdr_shuffled.Tis_inv == 0]))
print('------------------------')
...
csv_df_arg['Tis_inv'] = csv_df_arg['IN_SITU'].astype(int)
...
print('In situ:', len(csv_df_arg_shuffled[csv_df_arg_shuffled.Tis_inv == 1]))
print('Invassive:', len(csv_df_arg_shuffled[csv_df_arg_shuffled.Tis_inv == 0]))
print('------------------------')
...
csv_df_polesie['Tis_inv'] = csv_df_polesie['IN_SITU'].astype(int)
...
print('In situ:', len(csv_df_polesie_shuffled[csv_df_polesie_shuffled.Tis_inv == 1]))
print('Invassive:', len(csv_df_polesie_shuffled[csv_df_polesie_shuffled.Tis_inv == 0]))
print('------------------------')
...
Tis_inv_processed_isic = []
for i in range(len(csv_df_isic)):
    if 'in situ' in str(csv_df_isic.iloc[i]['mel_class']):
        Tis_inv_processed_isic.append(1)
    else:
        Tis_inv_processed_isic.append(0)
csv_df_isic['Tis_inv'] = Tis_inv_processed_isic
...
print('In situ:', len(csv_df_isic_shuffled[csv_df_isic_shuffled.Tis_inv == 1]))
print('Invassive:', len(csv_df_isic_shuffled[csv_df_isic_shuffled.Tis_inv == 0]))
print('------------------------')



csv_df_vdr_shuffled['Tis_inv'] = [str(x) for x in csv_df_vdr_shuffled['Tis_inv']]
csv_df_arg_shuffled['Tis_inv'] = [str(x) for x in csv_df_arg_shuffled['Tis_inv']]
csv_df_polesie_shuffled['Tis_inv'] = [str(x) for x in csv_df_polesie_shuffled['Tis_inv']]
csv_df_isic_shuffled['Tis_inv'] = [str(x) for x in csv_df_isic_shuffled['Tis_inv']]
...
for i in range(5):
	...
	print(train_df['Tis_inv'].value_counts())
	print(test_df['Tis_inv'].value_counts())

	train_df.to_csv(OUTPUT_PATH_FOLD+'train_df.csv', index = False, header=True, columns=['IMAGE_PATH','Tis_inv'])
	test_df.to_csv(OUTPUT_PATH_FOLD+'validation_df.csv', index = False, header=True, columns=['IMAGE_PATH','Tis_inv'])
	...
	# optimizer = optim.Adam(model.parameters(),lr=0.00001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.001, amsgrad=False)
	model.to(device)

...
parser.add_argument('-l', '--LEARNING_RATE', help='loss to use',type=float, default=0.00001)
...
print("\nTRAINING STUDENT WITH PSEUDOANNOTATIONS: \nN_EXP " + str(N_EXP), ", BATCH_SIZE " + str(BATCH_SIZE), ", EPOCHS " + str(EPOCHS), ", MODEL " + MODEL, ", LEARNING_RATE " + str(LEARNING_RATE), ", TASK " + TASK)
...
pretrained_models_path = '/home/jpdominguez/projects/BreslowTotal/src/full_supervision/train/models/' + MODEL + '/' + TASK + '/N_EXP_' + str(N_EXP) + '/'


models_path = '/home/jpdominguez/projects/BreslowTotal/src/semi_supervision/train/'
...
CSV_PATH_PSEUDO = '/home/jpdominguez/projects/BreslowTotal/src/semi_supervision/annotations/preds_' + TASK + '_' + MODEL + '_N_EXP_' + str(N_EXP) + '_majority.csv'
...
if TASK == 'Breslow' or TASK == 'InSitu':
    csv_df_pseudo.columns = ['IMAGE_PATH', 'WINNING_CLASS_0', 'WINNING_CLASS_1', 'CLASS']
elif TASK == 'Multiclass':
	csv_df_pseudo.columns = ['IMAGE_PATH', 'WINNING_CLASS_0', 'WINNING_CLASS_1', 'WINNING_CLASS_2', 'CLASS']
...
if TASK == 'Breslow':
    print('Breslow < 0.8:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 0]))
    print('Breslow >= 0.8:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 1]))
elif TASK == 'InSitu':
    print('In situ:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 0]))
    print('Invasive:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 1]))
elif TASK == 'Multiclass':
    print('In situ:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 0]))
    print('Breslow < 0.8:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 1]))
    print('Breslow >= 0.8:', len(csv_df_pseudo_shuffled[csv_df_pseudo_shuffled.CLASS == 2]))
print('------------------------\n')
...
for i in range(5):
	...
	finetuned_strongly_supervised_model_path = models_path_fold+'student_semisupervised.pt'
	...
	train_df.to_csv(models_path_fold+'train_df.csv', index = False, header=True, columns=['IMAGE_PATH','CLASS'])
	test_df.to_csv(models_path_fold+'validation_df.csv', index = False, header=True, columns=['IMAGE_PATH','CLASS'])
	...
	for i in range(len(train_dataset)):
		train_dataset[i,1] = str(train_dataset[i,1])
		train_dataset[i,2] = int(train_dataset[i,2])
	for i in range(len(valid_dataset)):
		valid_dataset[i,0] = str(valid_dataset[i,0])
		valid_dataset[i,1] = int(valid_dataset[i,1])

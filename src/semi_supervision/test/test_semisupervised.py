...
print("TESTING SEMI SUPERVISED MODEL, N_EXP ", N_EXP_str, ', BACKBONE:', MODEL)
...
models_path = '/home/jpdominguez/projects/BreslowTotal/src/semi_supervision/train/models/'
create_dir(models_path)
models_path = models_path+MODEL+'/'
create_dir(models_path)
...
for i in range(5):
	...
	model_path = OUTPUT_PATH_FOLD+'student_semisupervised.pt'
	...
	if NUM_CLASSES == 2:
		...
		plot_roc_curve_multiclass(y_true, output_preds=[output_preds_0, output_preds_1, output_preds_2],
						xlabel = 'False positive rate',
						ylabel = 'True positive rate',
						title        = "",
						metric       = np.array(roc_auc_score),
